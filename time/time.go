...
func formatEpochTime(time int64) string {
	str := strings.NewStringInt64(time)
	return str.SubstringEnd(10).AppendString(symbol.DOT).Append(str.SubstringBegin(10)).ToString()
}
...
func EpochToTime(epoch string) (time.Time, error) {
	...
	f, err := strings.NewString(epoch).ToFloat()
	if err != nil {
